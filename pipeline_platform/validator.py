...
def validate_config_dict(config: dict) -> None:
    ...
    All sections — source, destination, schedule, transform, runtime,
    logging, data_quality — are validated when present.

    Raises:
        ConfigValidationError: if any validation rule fails.
    """
    # Normalise to a consistent internal shape regardless of input format
    ...
    # Optional sections — only validated when present
    ...
    # Register the name after all checks pass
...
# ─────────────────────────────────────────────────────────────────────────────
# NORMALISATION  — flat - nested format bridge
# ─────────────────────────────────────────────────────────────────────────────
...
def _normalise(config: dict) -> dict:
    ...
    # Nested format — extract pipeline_name and owner from pipeline block
    ...
    # destination.write_mode maps to top-level load_mode
...
def _validate_source(source: dict) -> None:
    ...
    # Validate incremental_key type if present (flat or nested)
    incremental_key = source.get("incremental_key")
    if incremental_key is not None and not isinstance(incremental_key, str):
        raise ConfigValidationError("source.incremental_key must be a string")
...
def _validate_api_source(source: dict) -> None:
    ...
    if connection:
        # Nested format — validate connection block
        ...
        # Validate extraction block if present
        extraction = source.get("extraction", {})
        if extraction:
            _validate_extraction(extraction)

    else:
        # Flat format — just needs a top-level endpoint
...
def _validate_schedule(schedule: dict) -> None:
    ...
    # start_date format check if present
    ...
    if start_date is not None:
        try:
            ...
            raise ConfigValidationError(
                f"schedule.start_date must be a valid ISO date (YYYY-MM-DD), "
                f"got: '{start_date}'"
            )
...
def _validate_transform(transform: dict) -> None:
    ...
    if transform.get("enabled"):
        # Accept either 'sql' (flat) or 'sql_file' (brief format)
...
def _validate_logging(logging_cfg: dict) -> None:
    ...
    if log_level and log_level not in SUPPORTED_LOG_LEVELS:
        raise ConfigValidationError(
            f"logging.log_level must be one of {sorted(SUPPORTED_LOG_LEVELS)}, "
            f"got: '{log_level}'"
        )
