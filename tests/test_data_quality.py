...
def _checks(*args) -> QualityChecksConfig:
    """Build a QualityChecksConfig from QualityCheckConfig objects."""
...
def test_not_null_passes_when_no_nulls():
    ...
    run_quality_checks(df, checks)  # should not raise
...
def test_disabled_checks_do_not_run():
    """quality_checks.enabled=False means checks are skipped entirely."""
    df = pd.DataFrame({"customer_id": [None, None, None]})
    checks = QualityChecksConfig(
        enabled=False,
        checks=[_check("no_nulls", "customer_id", "not_null")],
    )
    run_quality_checks(df, checks)  # should not raise — checks disabled


def test_no_checks_configured_does_not_raise():
    """Empty checks list with enabled=True should not raise."""
...
# ─────────────────────────────────────────────────────────────────────────────
# end-to-end integration - quality failure logs FAILED run
# ─────────────────────────────────────────────────────────────────────────────
